...
if(!($stmt_ins = $conn->prepare("INSERT INTO timeline_events(id, timeline_id, description, deadline, completed) VALUES (?, ?, ?, ? ,?)"))) {
    echo "Prepare failed: (" . $conn->errno . ") " . $conn->error;
}
if(!($stmt_upd = $conn->prepare("UPDATE timeline_events SET description=?, deadline=?, completed=? WHERE timeline_id=".$jsonEvents->timeline_id))) {
    echo "Prepare failed: (" . $conn->errno . ") " . $conn->error;
}
...
for($i = 0; $i < count($descriptions); $i++) {
    if($new[$i]) {
        $zero = 0;
        if (!$stmt_ins->bind_param("isssi", $zero, $jsonEvents->timeline_id, $descriptions[$i], $deadlines[$i], $completions[$i])) {
    ...
    elseif($changes[$i]) {
        if (!$stmt_upd->bind_param("ssi", $descriptions[$i], $deadlines[$i], $completions[$i])) {
